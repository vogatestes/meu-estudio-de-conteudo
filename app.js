...
document.querySelectorAll(".toggle-btn[data-format]").forEach((btn) => {
  btn.addEventListener("click", () => {
    ...
    document.querySelectorAll(".chip[data-sub]").forEach((c) => c.classList.remove("selected"));
  });
});
document.querySelectorAll(".chip[data-sub]").forEach((chip) => {
  chip.addEventListener("click", () => {
    document.querySelectorAll(".chip[data-sub]").forEach((c) => c.classList.remove("selected"));
    chip.classList.add("selected");
    state.subformat = chip.dataset.sub;
...
setupSingleSelectChips("imageChips", "img", "imageChoice");
...
function renderPreview() {
  ...
  const canvasStyle = `background:linear-gradient(135deg, ${c.secondary} 0%, ${c.primary} 120%); color:white;`;
