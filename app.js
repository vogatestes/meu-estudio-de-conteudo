...
function goToStep(step) {
  ...
  const showPreviewOnMobile = ["design", "identity", "copy", "preview"].includes(step);
  document.getElementById("previewPanel").classList.toggle("show-on-mobile", showPreviewOnMobile);
...
document.querySelectorAll(".toggle-btn[data-format]").forEach((btn) => {
  btn.addEventListener("click", () => {
    ...
    // mantém a prévia (Feed/Story) sincronizada com o formato escolhido
    previewMode = state.format === "stories" ? "story" : "feed";
    document.querySelectorAll(".toggle-btn[data-preview]").forEach((b) => b.classList.toggle("active", b.dataset.preview === previewMode));
    renderPreview();
  });
});
...
document.querySelectorAll(".color-row").forEach((row) => {
  ...
  colorInput.addEventListener("input", () => { hexInput.value = colorInput.value; state.colors[key] = colorInput.value; renderPreview(); });
  hexInput.addEventListener("input", () => {
    if (/^#[0-9a-fA-F]{6}$/.test(hexInput.value)) { colorInput.value = hexInput.value; state.colors[key] = hexInput.value; renderPreview(); }
  });
});
...
document.getElementById("useLogoCheckbox").addEventListener("change", (e) => { state.useLogo = e.target.checked; renderPreview(); });
document.getElementById("logoPosition").addEventListener("change", (e) => { state.logoPosition = e.target.value; renderPreview(); });
...
document.getElementById("copyResult").addEventListener("input", () => {
...
// Cada um dos 20 padrões cai numa dessas famílias de layout — cada família
// desenha de um jeito diferente, tanto na tela quanto no PNG baixado.
const PATTERN_LAYOUT_MAP = {
  "Foto grande + título": "photo-bottom-bar",
  "Foto + bloco de texto": "photo-top-text-bottom",
  "Texto central": "centered",
  "Título gigante": "giant-title",
  "Editorial": "editorial",
  "Minimalista": "minimal",
  "Notícia": "news",
  "Curiosidade": "tag-centered",
  "Lista numerada": "list",
  "Antes × Depois": "split",
  "Pergunta + resposta": "qa",
  "Mito × Verdade": "split-label",
  "Comparação": "columns",
  "Infográfico": "timeline",
  "Card de estatística": "big-stat",
  "Frase de impacto": "centered",
  "Depoimento": "quote",
  "Checklist": "checklist",
  "Destaque de profissional": "avatar",
  "CTA / chamada para ação": "cta",
};
function currentLayout() {
  return PATTERN_LAYOUT_MAP[state.pattern] || "centered";
}
...
function renderLayoutHTML(layout, { text, title, cta, niche, hasPhoto }) {
  const bigFont = `font-family:var(--display);font-weight:600;line-height:1.3;position:relative;z-index:2;`;
  switch (layout) {
    case "photo-bottom-bar":
      return `<div style="position:absolute;left:0;right:0;bottom:0;padding:20px;background:linear-gradient(0deg, rgba(0,0,0,.75), transparent);${bigFont}font-size:19px;">${text}</div>`;
    case "photo-top-text-bottom":
      return `<div style="position:absolute;left:0;right:0;bottom:0;padding:20px;background:rgba(0,0,0,.55);${bigFont}font-size:17px;">${text}</div>`;
    case "giant-title":
      return `<div style="${bigFont}font-size:26px;align-self:flex-start;">${text}</div>`;
    case "editorial":
      return `<div style="align-self:flex-start;"><div style="width:28px;height:3px;background:currentColor;margin-bottom:10px;"></div><div style="${bigFont}font-size:16px;text-align:left;">${text}</div></div>`;
    case "minimal":
      return `<div style="display:flex;flex-direction:column;align-items:center;gap:14px;"><div style="width:10px;height:10px;border-radius:50%;background:currentColor;"></div><div style="${bigFont}font-size:15px;font-weight:400;">${text}</div></div>`;
    case "news":
      return `<div><span style="font-size:10px;font-weight:700;letter-spacing:.03em;background:rgba(255,255,255,.25);padding:4px 10px;border-radius:20px;">NOTÍCIA</span><div style="${bigFont}font-size:19px;margin-top:12px;">${text}</div></div>`;
    case "tag-centered":
      return `<div style="text-align:center;"><span style="font-size:10px;font-weight:700;letter-spacing:.03em;background:rgba(255,255,255,.25);padding:4px 10px;border-radius:20px;">CURIOSIDADE</span><div style="${bigFont}font-size:19px;margin-top:12px;">${text}</div></div>`;
    case "list": {
      const items = splitIntoItems(text, 4);
      return `<div style="display:flex;flex-direction:column;gap:12px;">${items.map((it, i) => `<div style="display:flex;gap:10px;align-items:flex-start;"><span style="font-family:var(--display);font-weight:700;opacity:.7;">${String(i + 1).padStart(2, "0")}</span><span style="font-size:14px;line-height:1.35;">${it}</span></div>`).join("")}</div>`;
    }
    case "checklist": {
      const items = splitIntoItems(text, 4);
      return `<div style="display:flex;flex-direction:column;gap:12px;">${items.map((it) => `<div style="display:flex;gap:10px;align-items:flex-start;"><span style="width:16px;height:16px;flex-shrink:0;border-radius:4px;background:rgba(255,255,255,.9);color:${state.colors.secondary};display:flex;align-items:center;justify-content:center;font-size:11px;margin-top:1px;">✓</span><span style="font-size:14px;line-height:1.35;">${it}</span></div>`).join("")}</div>`;
    }
    case "split": {
      const half = hasPhoto ? "" : `background:${state.colors.secondary};`;
      return `<div style="position:absolute;inset:0;display:flex;">
          <div style="flex:1;${half}display:flex;align-items:center;justify-content:center;padding:16px;position:relative;z-index:2;"><span style="font-size:10px;font-weight:700;background:rgba(0,0,0,.4);padding:4px 10px;border-radius:20px;">ANTES</span></div>
          <div style="flex:1;background:${state.colors.primary};display:flex;align-items:center;justify-content:center;padding:16px;position:relative;z-index:2;"><span style="font-size:10px;font-weight:700;background:rgba(0,0,0,.25);padding:4px 10px;border-radius:20px;">DEPOIS</span></div>
        </div>
        <div style="position:absolute;left:16px;right:16px;bottom:20px;${bigFont}font-size:15px;background:rgba(0,0,0,.55);padding:12px;border-radius:10px;z-index:3;">${text}</div>`;
    }
    case "split-label": {
      const mid = Math.floor(text.length / 2);
      const cut = text.indexOf(" ", mid) > -1 ? text.indexOf(" ", mid) : mid;
      const left = text.slice(0, cut).trim();
      const right = text.slice(cut).trim();
      return `<div style="position:absolute;inset:0;display:flex;">
          <div style="flex:1;background:${state.colors.accent};display:flex;flex-direction:column;justify-content:center;padding:18px;gap:8px;"><span style="font-size:11px;font-weight:700;">MITO</span><span style="font-size:14px;line-height:1.3;">${left}</span></div>
          <div style="flex:1;background:var(--green);display:flex;flex-direction:column;justify-content:center;padding:18px;gap:8px;"><span style="font-size:11px;font-weight:700;">VERDADE</span><span style="font-size:14px;line-height:1.3;">${right}</span></div>
        </div>`;
    }
    case "columns": {
      const words = text.split(" ");
      const third = Math.ceil(words.length / 3);
      const cols = [words.slice(0, third).join(" "), words.slice(third, third * 2).join(" "), words.slice(third * 2).join(" ")];
      const bg = [state.colors.secondary, state.colors.primary, state.colors.accent];
      return `<div style="position:absolute;inset:0;display:flex;">${cols.map((col, i) => `<div style="flex:1;background:${bg[i]};display:flex;align-items:center;justify-content:center;padding:12px;"><span style="font-size:13px;text-align:center;line-height:1.3;">${col}</span></div>`).join("")}</div>`;
    }
    case "timeline": {
      const items = splitIntoItems(text, 3);
      return `<div style="display:flex;flex-direction:column;gap:0;">${items.map((it, i) => `
          <div style="display:flex;gap:12px;">
            <div style="display:flex;flex-direction:column;align-items:center;">
              <span style="width:12px;height:12px;border-radius:50%;background:currentColor;flex-shrink:0;"></span>
              ${i < items.length - 1 ? `<span style="width:2px;flex:1;background:rgba(255,255,255,.4);min-height:24px;"></span>` : ""}
            </div>
            <span style="font-size:13px;line-height:1.35;padding-bottom:18px;">${it}</span>
          </div>`).join("")}</div>`;
    }
    case "big-stat": {
      const match = text.match(/\d+%?/);
      const stat = match ? match[0] : "92%";
      const label = match ? text.replace(match[0], "").replace(/^[:\-–\s]+/, "") : text;
      return `<div style="text-align:center;"><div style="font-family:var(--display);font-weight:700;font-size:52px;">${stat}</div><div style="font-size:14px;margin-top:8px;">${label}</div></div>`;
    }
    case "quote":
      return `<div style="text-align:center;"><div style="font-family:var(--display);font-size:38px;line-height:.6;">"</div><div style="${bigFont}font-weight:400;font-style:italic;font-size:16px;margin-top:10px;">${text}</div><div style="font-size:11px;margin-top:14px;opacity:.8;">— Equipe de ${niche || "atendimento"}</div></div>`;
    case "avatar":
      return `<div style="display:flex;gap:12px;align-items:center;"><div style="width:44px;height:44px;border-radius:50%;background:rgba(255,255,255,.25);flex-shrink:0;display:flex;align-items:center;justify-content:center;font-family:var(--display);font-weight:700;">${(niche || "P")[0]}</div><div><div style="font-size:11px;font-weight:700;opacity:.85;">${niche || "Profissional"}</div><div style="font-size:14px;line-height:1.3;margin-top:4px;">${text}</div></div></div>`;
    case "cta":
      return `<div style="text-align:center;"><div style="${bigFont}font-size:20px;">${text}</div>${cta ? `<div style="margin-top:16px;display:inline-block;padding:12px 24px;border-radius:24px;background:rgba(255,255,255,.95);color:${state.colors.secondary};font-weight:700;font-size:13px;">${cta} →</div>` : ""}</div>`;
    case "qa": {
      const qIdx = text.indexOf("?");
      const q = qIdx > -1 ? text.slice(0, qIdx + 1) : title || text;
      const a = qIdx > -1 ? text.slice(qIdx + 1).trim() : text;
      return `<div><div style="${bigFont}font-size:18px;">${q}</div><div style="width:100%;height:1px;background:rgba(255,255,255,.4);margin:14px 0;"></div><div style="font-size:14px;line-height:1.4;">${a || text}</div></div>`;
    }
    case "centered":
    default:
      return `<div style="${bigFont}font-size:19px;text-align:left;">${text}</div>`;
  }
}
...
function renderPreview() {
  ...
  const layoutContent = renderLayoutHTML(layout, {
    text: slides[state.currentSlide] || "",
    title: r.title || "",
    cta: r.cta || "",
    niche: state.niche,
    hasPhoto,
  ...
  if (previewMode === "feed") {
    mockup.innerHTML = `
      ...
      <div class="ig-canvas" style="${canvasStyle}">
        ${logoHtml()}
        <div style="position:relative;z-index:2;width:100%;height:100%;">${layoutContent}</div>
...
async function drawSlideToCanvas(slideIndex, mode) {
  ...
  await document.fonts.load(`700 ${Math.round(13 * scale)}px "Space Grotesk"`);
  await document.fonts.ready;

  function drawWrappedText(str, x, y, maxWidth, fontSize, opts = {}) {
    const { weight = 600, family = "Fraunces, serif", color = "#ffffff", align = "left", lineHeightMult = 1.3, italic = false } = opts;
    ctx.font = `${italic ? "italic " : ""}${weight} ${fontSize}px "${family}"`;
    ctx.fillStyle = color;
    ctx.textBaseline = "middle";
    ctx.textAlign = align;
    const lines = wrapCanvasLines(ctx, str, maxWidth);
    const lineHeight = fontSize * lineHeightMult;
    lines.forEach((line, i) => ctx.fillText(line, x, y + i * lineHeight));
    ctx.textAlign = "left";
    return lines.length * lineHeight;
  }
  function drawPill(label, x, y, opts = {}) {
    const { bg = "rgba(255,255,255,.25)", color = "#fff", fontSize = Math.round(11 * scale) } = opts;
    ctx.font = `700 ${fontSize}px "Space Grotesk", sans-serif`;
    const padX = Math.round(10 * scale), padY = Math.round(6 * scale);
    const tw = ctx.measureText(label).width;
    const w = tw + padX * 2, h = fontSize + padY * 2, radius = h / 2;
    ctx.fillStyle = bg;
    ctx.beginPath();
  ...
  async function drawBackgroundPhotoOrGradient() {
    if (hasPhoto) {
      try {
        ...
        if (imgRatio > canvasRatio) { drawH = H; drawW = H * imgRatio; dx = (W - drawW) / 2; dy = 0; }
        else { drawW = W; drawH = W / imgRatio; dx = 0; dy = (H - drawH) / 2; }
  ...
  const ownsBackground = ["split", "split-label", "columns"].includes(layout);
  if (!ownsBackground) await drawBackgroundPhotoOrGradient();

  const titleSize = Math.round((isStory ? 22 : 19) * scale);
  ...
  if (layout === "split") {
    ...
    if (hasPhoto) { try { const bg = await loadImage(state.contentImageDataUrl); ctx.drawImage(bg, 0, 0, W / 2, H); } catch {} }
    drawPill("ANTES", paddingX, H * 0.42, {});
    drawPill("DEPOIS", W / 2 + paddingX, H * 0.42, {});
    const boxY = H - Math.round(140 * scale);
    ctx.fillStyle = "rgba(0,0,0,.55)";
    ctx.fillRect(paddingX, boxY, W - paddingX * 2, Math.round(120 * scale));
    drawWrappedText(text, paddingX + 16, boxY + 60 * scale, W - paddingX * 2 - 32, bodySize, {});
  } else if (layout === "split-label") {
    const mid = Math.floor(text.length / 2);
    const cut = text.indexOf(" ", mid) > -1 ? text.indexOf(" ", mid) : mid;
    const left = text.slice(0, cut).trim();
    const right = text.slice(cut).trim();
    ...
    ctx.fillRect(W / 2, 0, W / 2, H);
    drawWrappedText("MITO", paddingX, H * 0.38, W / 2 - paddingX * 1.5, Math.round(13 * scale), { family: "Space Grotesk", weight: 700 });
    drawWrappedText(left, paddingX, H * 0.48, W / 2 - paddingX * 1.5, bodySize, {});
    drawWrappedText("VERDADE", W / 2 + paddingX, H * 0.38, W / 2 - paddingX * 1.5, Math.round(13 * scale), { family: "Space Grotesk", weight: 700 });
    drawWrappedText(right, W / 2 + paddingX, H * 0.48, W / 2 - paddingX * 1.5, bodySize, {});
  } else if (layout === "columns") {
    const words = text.split(" ");
    const third = Math.ceil(words.length / 3);
    const cols = [words.slice(0, third).join(" "), words.slice(third, third * 2).join(" "), words.slice(third * 2).join(" ")];
    const bgColors = [c.secondary, c.primary, c.accent];
    const colW = W / 3;
    cols.forEach((col, i) => {
      ctx.fillStyle = bgColors[i];
      ctx.fillRect(colW * i, 0, colW, H);
      drawWrappedText(col, colW * i + colW / 2, H / 2, colW - paddingX, bodySize, { align: "center" });
    });
  } else if (layout === "photo-bottom-bar" || layout === "photo-top-text-bottom") {
    const boxH = Math.round((layout === "photo-bottom-bar" ? 220 : 260) * scale);
    ctx.fillStyle = layout === "photo-bottom-bar" ? "rgba(0,0,0,.65)" : "rgba(0,0,0,.55)";
    ctx.fillRect(0, H - boxH, W, boxH);
    drawWrappedText(text, paddingX, H - boxH / 2, W - paddingX * 2, titleSize, {});
  } else if (layout === "giant-title") {
    drawWrappedText(text, paddingX, H * 0.3, W - paddingX * 2, Math.round(titleSize * 1.35), { align: "left" });
  } else if (layout === "editorial") {
    ctx.fillStyle = "#ffffff";
    ctx.fillRect(paddingX, H * 0.38, Math.round(40 * scale), Math.round(4 * scale));
    drawWrappedText(text, paddingX, H * 0.45, W - paddingX * 2, Math.round(16 * scale), {});
  } else if (layout === "minimal") {
    ctx.fillStyle = "#ffffff";
    ctx.beginPath(); ctx.arc(W / 2, H * 0.42, Math.round(7 * scale), 0, Math.PI * 2); ctx.fill();
    drawWrappedText(text, W / 2, H * 0.5, W * 0.7, Math.round(15 * scale), { align: "center", weight: 400 });
  } else if (layout === "news" || layout === "tag-centered") {
    const align = layout === "tag-centered" ? "center" : "left";
    const x = align === "center" ? W / 2 : paddingX;
    drawPill(layout === "news" ? "NOTÍCIA" : "CURIOSIDADE", align === "center" ? W / 2 - 60 * scale : paddingX, H * 0.36, {});
    drawWrappedText(text, x, H * 0.46, W - paddingX * 2, titleSize, { align });
  } else if (layout === "list" || layout === "checklist") {
    const items = splitIntoItems(text, 4);
    const lineH = Math.round(bodySize * 1.4);
    let y = H / 2 - (items.length * lineH) / 2;
    items.forEach((it, i) => {
      if (layout === "list") {
        drawWrappedText(String(i + 1).padStart(2, "0"), paddingX, y, 50 * scale, bodySize, { family: "Fraunces", weight: 700 });
        drawWrappedText(it, paddingX + 50 * scale, y, W - paddingX * 2 - 50 * scale, bodySize, {});
      } else {
        ctx.fillStyle = "rgba(255,255,255,.9)";
        ctx.fillRect(paddingX, y - 8 * scale, 20 * scale, 20 * scale);
        ctx.fillStyle = c.secondary;
        ctx.font = `700 ${Math.round(13 * scale)}px "Space Grotesk"`;
        ctx.textBaseline = "middle";
        ctx.fillText("✓", paddingX + 5 * scale, y + 2 * scale);
        drawWrappedText(it, paddingX + 32 * scale, y, W - paddingX * 2 - 32 * scale, bodySize, {});
      }
      y += lineH + 14 * scale;
    });
  } else if (layout === "timeline") {
    const items = splitIntoItems(text, 3);
    const lineH = Math.round(bodySize * 1.8);
    let y = H / 2 - (items.length * lineH) / 2;
    items.forEach((it) => {
      ctx.fillStyle = "#ffffff";
      ctx.beginPath(); ctx.arc(paddingX + 6 * scale, y, 6 * scale, 0, Math.PI * 2); ctx.fill();
      drawWrappedText(it, paddingX + 24 * scale, y, W - paddingX * 2 - 24 * scale, bodySize, {});
      y += lineH + 20 * scale;
    });
  } else if (layout === "big-stat") {
    const match = text.match(/\d+%?/);
    const stat = match ? match[0] : "92%";
    const label = match ? text.replace(match[0], "").replace(/^[:\-–\s]+/, "") : text;
    drawWrappedText(stat, W / 2, H * 0.42, W * 0.8, Math.round(64 * scale), { family: "Fraunces", weight: 700, align: "center" });
    drawWrappedText(label, W / 2, H * 0.54, W * 0.75, bodySize, { align: "center" });
  } else if (layout === "quote") {
    drawWrappedText('"', W / 2, H * 0.32, W * 0.5, Math.round(60 * scale), { align: "center" });
    drawWrappedText(text, W / 2, H * 0.44, W * 0.75, Math.round(16 * scale), { align: "center", italic: true, weight: 400 });
    drawWrappedText(`— Equipe de ${state.niche || "atendimento"}`, W / 2, H * 0.58, W * 0.7, Math.round(11 * scale), { align: "center" });
  } else if (layout === "avatar") {
    const cx = paddingX + 22 * scale, cy = H * 0.44;
    ctx.fillStyle = "rgba(255,255,255,.25)";
    ctx.beginPath(); ctx.arc(cx, cy, 22 * scale, 0, Math.PI * 2); ctx.fill();
    ctx.fillStyle = "#fff";
    ctx.font = `700 ${Math.round(16 * scale)}px "Fraunces"`;
    ctx.textAlign = "center"; ctx.textBaseline = "middle";
    ctx.fillText((state.niche || "P")[0], cx, cy);
    ctx.textAlign = "left";
    drawWrappedText(state.niche || "Profissional", paddingX + 56 * scale, cy - 12 * scale, W - paddingX * 2 - 56 * scale, Math.round(11 * scale), { family: "Space Grotesk", weight: 700 });
    drawWrappedText(text, paddingX + 56 * scale, cy + 12 * scale, W - paddingX * 2 - 56 * scale, bodySize, {});
  } else if (layout === "cta") {
    const h1 = drawWrappedText(text, W / 2, H * 0.42, W * 0.75, Math.round(titleSize * 1.1), { align: "center" });
    if (r.cta) {
      ctx.font = `700 ${Math.round(13 * scale)}px "Space Grotesk"`;
      const tw = ctx.measureText(r.cta + " →").width;
      const bx = W / 2 - tw / 2 - 20 * scale, by = H * 0.42 + h1 / 2 + 24 * scale;
      const bw = tw + 40 * scale, bh = 44 * scale;
      ctx.fillStyle = "#ffffff";
      ctx.beginPath();
      const radius = bh / 2;
      ...
      ctx.fillStyle = c.secondary;
      ctx.textAlign = "center"; ctx.textBaseline = "middle";
      ctx.fillText(r.cta + " →", W / 2, by + bh / 2);
      ctx.textAlign = "left";
    }
  } else if (layout === "qa") {
    const qIdx = text.indexOf("?");
    const q = qIdx > -1 ? text.slice(0, qIdx + 1) : (r.title || text);
    const a = qIdx > -1 ? text.slice(qIdx + 1).trim() : text;
    const h1 = drawWrappedText(q, paddingX, H * 0.38, W - paddingX * 2, titleSize, {});
    const lineY = H * 0.38 + h1 / 2 + 20 * scale;
    ctx.fillStyle = "rgba(255,255,255,.4)";
    ctx.fillRect(paddingX, lineY, W - paddingX * 2, 2);
    drawWrappedText(a || text, paddingX, lineY + 50 * scale, W - paddingX * 2, bodySize, {});
  } else {
    // "centered" — padrão para Texto central, Frase de impacto, e fallback
    drawWrappedText(text, paddingX, H / 2, W - paddingX * 2, titleSize, {});
  }

  // CTA extra no rodapé (só formatos story que não sejam o layout "cta", que já desenha o próprio botão)
  if (isStory && r.cta && layout !== "cta") {
    drawPill(r.cta, paddingX, H - Math.round(90 * scale), { bg: "#ffffff", color: c.secondary, fontSize: Math.round(13 * scale) });
  }
